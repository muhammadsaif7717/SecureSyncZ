...
export function RestoreVaultModal({ isOpen }: { isOpen: boolean }) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => {}}>
      <DialogContent 
        className="w-[calc(100vw-2rem)] sm:w-full [&>button]:hidden overflow-hidden p-0"
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center p-6 sm:p-8">
          <div className="absolute top-4 right-4">
            <Button variant="ghost" size="sm" onClick={logout} className="text-slate-500 hover:text-red-500 flex gap-2">
          ...
          <DialogHeader className="mb-6">
            ...
            <DialogDescription className="text-center text-sm text-slate-500 dark:text-slate-400">
              Please enter your 6-digit passkey and your 64-character Secret Key to restore access on this device.
            </DialogDescription>
          </DialogHeader>

          <form onSubmit={handleVerify} className="flex w-full flex-col items-center space-y-6">
            <div className="w-full space-y-2">
              <Label htmlFor="secretKey" className="text-sm font-medium text-slate-700 dark:text-slate-300">
                Secret Key
              </Label>
              <Input
                id="secretKey"
                ...
                className="font-mono text-sm h-11 bg-white/60 dark:bg-white/5"
                required
              />
            </div>

            <div className="w-full space-y-2 flex flex-col items-center">
              <Label className="text-sm font-medium text-slate-700 dark:text-slate-300 self-start">
                Passkey
              </Label>
              <InputOTP
                maxLength={6}
                ...
                <InputOTPGroup className="gap-2">
                  {[...Array(6)].map((_, i) => (
                    <InputOTPSlot
                      key={i}
                      index={i}
                      className="h-10 w-10 sm:h-12 sm:w-12 rounded-md border-slate-200 bg-white/60 text-base sm:text-lg dark:border-white/10 dark:bg-white/5"
                    />
                  ))}
                </InputOTPGroup>
              </InputOTP>
            ...
            <Button
              type="submit"
              disabled={passkey.length !== 6 || isVerifying || secretKeyInput.length !== 64}
